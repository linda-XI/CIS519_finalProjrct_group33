...
def extract(input_dir=None,crop_location=None,output=None,crop_width=None,crop_height=None,nms_type_arg='CPU_NMS',model_path='/content/gdrive/MyDrive/519project/CIS519_FINAL_project-20211202T191237Z-001/CIS519_FINAL_project/anime-face-detector-master/model/res101_faster_rcnn_iter_60000.ckpt',nms_thresh=0.3,conf_thresh=0.8):
    ...
    assert os.path.exists(input_dir), 'The input path does not exists'

    if os.path.isdir(input_dir):
        files = load_file_from_dir(input_dir)
    else:
        files = [input_dir]
    file_len = len(files)
    print("type is")
    ...
    if nms_type_arg == 'PY_NMS':
        nms_type = NMSType.PY_NMS
    elif nms_type_arg == 'CPU_NMS':
        nms_type = NMSType.CPU_NMS
    elif nms_type_arg == 'GPU_NMS':
    ...
    for idx, file in enumerate(files):
        ...
        for i in range(scores.shape[0]):
            ...
            if output is None and crop_location is None:
                cv2.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)

            if crop_location:
                print("croping idx"+str(idx))
                # print("crop name"+str(args.start_output_number))
                ...
                pic_name = file[file.rindex("/") + 1 : file.rindex(".")] + "_"+str(same_count)                             

                # cv2.imwrite(args.crop_output_image_location + str(args.start_output_number) + ".jpg", cropped_image)
                cv2.imwrite(crop_location + pic_name + ".jpg", cropped_image)
